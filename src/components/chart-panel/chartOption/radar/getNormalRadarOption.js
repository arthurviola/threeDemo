...
export function getNormalRadarOption (data, config) {
  ...
  return {
    tooltip: {
      ...
      trigger: 'item',
      formatter: function () {
        return opt.indicator[0].name + '：' + data[0].value[0] + `</br>` + opt.indicator[1].name + '：' + data[0].value[1] + `</br>` + opt.indicator[2].name + '：' + data[0].value[2] + `</br>` + opt.indicator[3].name + '：' + data[0].value[3] + `</br>` + opt.indicator[4].name + '：' + data[0].value[4]
      }
    },
    radar: {
      center: ['50%', '53%'], // 外圆的位置
